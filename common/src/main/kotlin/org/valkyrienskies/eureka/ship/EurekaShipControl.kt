...
class EurekaShipControl : ShipPhysicsListener, ServerTickListener {
    ...
    override fun physTick(physShip: PhysShip, physLevel: PhysLevel) {
        ...
        // === Per-set "hold the opposite input to cancel", + auto-off when every set is canceled ===
        // Holding the input opposite a latched set's influence for the set's configured seconds unlatches just
        // that set (back to free/live), leaving the others. When the LAST latched set is canceled, the whole
        // cruise switches off with the normal message -- exactly as if C were pressed.
        ...
        if (!vanillaControls && isCruising) {
            ...
            if (!cruiseHorizontalActive && !cruiseVerticalActive && !cruiseTurning) {
                isCruising = false
                showCruiseStatus()
                clearCruiseLatches()
            }
    ...
    private fun getControlData(player: SeatedControllingPlayer): ControlData {
        ...
        if (!wasCruisePressed && player.cruise) {
            if (!isCruising) {
                ...
                // straightens, vertical eases to a hover). If NOTHING is engaged, don't turn cruise on at all.
                ...
                // the set would be stuck (uncancellable, blocking auto-off); the turn key stays free/live.
                val latchT = currentControlData.leftImpulse != 0.0f && EurekaConfig.SERVER.enableTurnCruise
                if (latchH || latchV || latchT) {
                    isCruising = true
    ...
    private fun applyPlayerControl(control: ControlData, physShip: PhysShip, thrustMultiplier: Double) {
        ...
        if (vanillaControls) {
            ...
            val hardMax = TURN_OMEGA_MAX_LINEAR / r

            idealAlphaY = when {
                turnKeyHeld && turnLatched ->
                    // Case B: a held/tapped turn ADDS influence in its direction (micro-adjust the orbit), capped
                    // at the hard max. Never cancels cruise; same-dir taps tighten even past the base cap.
                    if ((dir > 0.0 && omega.y() < hardMax) || (dir < 0.0 && omega.y() > -hardMax)) chase * dir else 0.0
                turnKeyHeld ->
                    // Case A (free turning): ramp toward the turnSpeed cap in the held direction.
                    (effCap * dir - omega.y()).coerceIn(-chase, chase)
                turnLatched ->
                    // Case B released: maintain the locked orbit rate (a held/tapped adjustment stuck on release).
                    ((cruiseTurnOmega ?: 0.0) - omega.y()).coerceIn(-chase, chase)
                else ->
                    // Case A released (or not cruising): brake back to center.
