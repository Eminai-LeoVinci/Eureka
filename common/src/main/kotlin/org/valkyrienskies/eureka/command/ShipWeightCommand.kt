 ...
 * reads EurekaConfig.
 *
 * The balloon threshold mirrors EurekaShipControl's own airborne test:
 * balloons * (massPerBalloon * g * balloonLiftMultiplier) >= mass * g. The floater thresholds come
...
object ShipWeightCommand {
    ...
    private fun getShipWeight(ctx: CommandContext<CommandSourceStack>, floater: Boolean): Int {
        ...
        if (floater) {
            ...
            val needed = ceil(mass / liftPerBalloon).toInt() + SAFETY_MARGIN
            val balloons = control?.balloons ?: 0

            msg.append(newline())
            msg.append(
                Component.literal("Balloons needed to fly: ${fmtInt(needed)} (each lifts ${fmtKg(liftPerBalloon)})")
                    .withStyle(ChatFormatting.AQUA)
            )
            ...
            msg.append(
                Component.literal(
                    "Balloons on ship: ${fmtInt(balloons)}" +
                        if (enough) " -- enough to fly!" else " -- need ${fmtInt(needed - balloons)} more"
